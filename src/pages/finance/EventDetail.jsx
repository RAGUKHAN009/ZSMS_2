...
export default function EventDetail() {
  ...
  if (!ev) return <div className="empty-state">Event not found.</div>;
  const total = (ev.expenses || []).reduce((sum, r) => sum + Number(r.total_expense || 0), 0);
  const [sig1, sig2] = signaturesForSection("FINANCE");

  const finalize = async () => {
    setBusy(true);
    try {
      setCollection("events", (rows) => rows.map((row) => (row.id === id ? { ...row, status: "active" } : row)));
      addAuditLog({ actor: user.full_name, action: "event_expense_finalized", entity: "events", entityId: id, newState: "active" });
  ...
  const revert = async () => {
    setBusy(true);
    try {
      setCollection("events", (rows) => rows.map((row) => (row.id === id ? { ...row, status: "draft" } : row)));
      addAuditLog({ actor: user.full_name, action: "event_expense_reverted", entity: "events", entityId: id, comment });
      showToast("Reverted to FS for correction.");
    } finally { setBusy(false); }
  };
    ...
    <div>
      ...
      <Card>
        <div className="row" style={{ justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            <div style={{ fontSize: 20, fontWeight: 800 }}>{ev.event_name}</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{ev.event_date}</div>
          </div>
        ...
        <div className="table-wrap" style={{ marginTop: 16 }}>
          <table className="data-table">
            <thead><tr><th>Product</th><th>Vendor</th><th>Total</th></tr></thead>
            <tbody>
              {(ev.expenses || []).map((row) => (
                <tr key={row.id}><td>{row.product_name}</td><td>{row.vendor_name}</td><td>Rs {Number(row.total_expense).toLocaleString()}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
        ...
        {canReviewFinance(role) && ev.status === "under_gs_review" && (
          <div className="no-print">
            <div className="field"><label>Comment (if reverting)</label><textarea className="input" style={{ minHeight: 60 }} value={comment} onChange={(e) => setComment(e.target.value)} /></div>
